...
class AuthenticationController(val authenticationService: AuthenticationService) {
    ...
    @PutMapping("/api/v1/authentication/userInfo")
    @ApiOperation(value = "Update userInfo ")
    fun updateUserInfo(@RequestHeader("Authorization") authorizationToken: String?,@RequestBody body: UserInfoDto): UserInfoDto {
        if (authorizationToken != null) {
            return authenticationService.updateUserInfo(jwtTokenManager.getUserFromTokenWithBearer(authorizationToken),body)
        } else {
            throw HttpClientErrorException(HttpStatus.FORBIDDEN)
        }
    }
